<div class="carousel-item" >
    <div class="row align-items-center">
            <div class="col-12 text-center text-lg-start col-lg-4 animate__animated animate__fadeInUp animate__delay-05s">
                <h2 class="text-primary fw-bold">${content.title}</h2>
                <#if (content.description)??>
                <p clas="text-muted">${content.description}</p>
                <#else></#if>
                <a href="${content.description}" class="btn btn-primary d-inline-block ">Buscar centros</a>
            </div>
